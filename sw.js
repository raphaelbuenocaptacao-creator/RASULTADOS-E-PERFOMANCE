...
const CACHE=`${CACHE_PREFIX}v15-private-vary-range-safe-shell`;
...
self.addEventListener('fetch',event=>{
  ...
  if(request.mode==='navigate'){
    event.respondWith(
      fetch(new Request(request,{cache:'no-store',credentials:'same-origin',redirect:'error'}))
  ...
  event.respondWith(
    caches.match(request).then(cached=>cached || fetch(new Request(request,{cache:'no-store',credentials:'omit',redirect:'error'})).then(response=>{
